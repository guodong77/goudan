...
if(conf.debugflag){  
  switch(sd.debug)
      ...
      case 1: //假数据，返回成功；
        ...
        url = conf.rooturl  +path+cid+'.json'; 
        break;
      case 0: //假数据，返回失败；
        url = conf.rooturl +'err.json';
        break;
      case -2://假数据，返回无权限；
      url = conf.rooturl +'noauth.json';          
        break;
      case -1://假数据，返回未登录；
        url = conf.rooturl +'nologin.json';
        break;
      default:
        break;
      } 
...
get:function (sendurl ,senddata ,callback) {
 ...
 fetch(sendurl).then(response => {console.log('url:',sendurl,'参数',senddata,'返回json数据:',response.body);callback(response.body);}).catch((err)=>{
  console.log(err)})
}, 
...
fetch(sendurl).then(response => {
  console.log('url:',sendurl,'json数据:',response.body);
