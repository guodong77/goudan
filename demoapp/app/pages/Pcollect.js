          ...
          <View style={styles.BuildContent}>
            ...
            <View style={styles.BuildDetail}>
              <View style={styles.BuildDetailTextView}>
                <Text numberOfLines={3} style={styles.BuildDetailText} onPress={() => {Actions.Builddetail({name: 'Brent'})}}>虽然常用的JS编辑器很多，但由于RN大量使用和es6语法，目前只有sublime text通过插件和本webstorm（10以上版本）提供了良好的支持。笔者推荐webstorm，因为它有更完善的语法提示和补全。另外虽然主要的业务逻辑是使用js开发，但仍然要依赖于原生的编译/调试环境，所以你还需要同时运行Xcode（iOS）或Android Studio（android）等。
                </Text>
