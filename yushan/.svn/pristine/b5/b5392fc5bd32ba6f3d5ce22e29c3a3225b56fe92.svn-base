...
$url = "https://graph.qq.com/oauth2.0/token?grant_type=authorization_code&client_id=101271490&client_secret=1e75abed5fdbb6c539134f538f73128c&code=".$code."&redirect_uri=http%3A%2F%2Fwww.ysjianzhu.com%2Fapi%2Fver1%2FLogin%2Fqqcbk"; 
$re = getcurlstr($url);
...
public function pcwecallback(){ 
  //ys建筑网站 的代码
  $code = $_GET['code']; 
  $url = 'https://api.weixin.qq.com/sns/oauth2/access_token?appid=wx7a5d92e1931e21ae&secret=36d7a27fce35f492eed5c7398474562b&code='.$code.'&grant_type=authorization_code';
  if($code){ 
    //获取accestoken
  ...
  if ($_GET['state'] == 'bindwx') {   //个人信息绑定微信
    $uid = getuser()['uid'];
    $save = Db::connect("DB_Config_1")->name('user')->where(array('uid'=>$uid))->update($data);
    // if ($save) {
      header("location: http://www.ysjianzhu.com/main.html#/user/info");
      exit;
    // }
    ...
    //如果有这个人；设置登录状态，直接跳转到后台页面；
        ...
        // }
      Db::connect("DB_Config_1")->name('user')->where(array('uid'=>$user['uid']))->update($data);
      lgfun($user,2);
    header("location: http://www.ysjianzhu.com/main.html");
      exit;
    }else{
 ...
 public function qqweaddbind(){ 
  ...
  if($user){
    if($logtype==3){//判断是微信还是qq
     ...
     $re=Db::connect("DB_Config_1")->name('user')->where(['uid'=>$user['uid']])->update($data);
     $userdata = Db::connect("DB_Config_1")->name('user')->where(array('uid'=>$user['uid']))->find();
