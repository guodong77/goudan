...
class AdminMessageController extends AdminBaseController
{
    ...
    public function addmsg()
    {
        if ($this->request->isPost()) {
            if(DB::name('api_store')->where("name='".$_POST['name']."'")->find()){
                $this->error("平台已存在");
            }
            $_POST['create_time']=time();
            $result             = DB::name('api_store')->insertGetId($_POST);
            if ($result !== false) {
    ...
    public function statusSwitch(){
         if ($this->request->isPost()) {
            ...
            $re=DB::name('api_store')->where("id='".input('id')."'")->update($data);
            if($re){
