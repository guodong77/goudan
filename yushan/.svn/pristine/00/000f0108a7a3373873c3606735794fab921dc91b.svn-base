...
// 生成codeid， 
function setcodeid($arr,$num,$isdel=true,$param=null){
	$id = gettable_id($num)['id'];
	/*生成codeid同时，删除原始ID； 0*6  */
	foreach ($arr as &$value) {
		if($param){
			$value[$param] = jiami($num.'*'.$value[$id]);
		}else{
...
if($value['byriqi']>0&&$value['ksornot']==1){	
    ...
    if($kst<28){$value['ks']=3;
       if(isset($value['bybuwei']))$value['ksdesc'] .='【'.$value['bybuwei']."】抗渗龄期小于28天，不符合规范值;";}
    if($kst>90){$value['ks']=3;
       if(isset($value['bybuwei']))$value['ksdesc'] .='【'.$value['bybuwei']."】抗渗龄期大于90天，不符合规范值;";}
...
{
	$a = Db::connect("DB_Config_1")->name('songjian')->where(['sid'=>$id,'isdel'=>0,'byriqi'=>['>',0]])->order('byriqi asc')->limit(1)->select();
	$btime = $a[0]['byriqi'];
	if ($btime) {
	  ...
	  foreach ($arr as $ke => $valu) { 
			...
			$dzchangjiaarr = explode(",",$valu['dzchangjia']); 
	     	for ($i=0; $i < $valu['dzzushu']; $i++) { 
	     		$qv = $valu;
	     		isset($dzguigearr[$i])?$qv['dzguige'] = $dzguigearr[$i]:''; 
	     		isset($dzjgarr[$i])?$qv['dzjg'] = $dzjgarr[$i]:''; 
	     		isset($dzpaihaoarr[$i])?$qv['dzpaihao'] = $dzpaihaoarr[$i]:''; 
	     		isset($dzbharr[$i])?$qv['dzbh'] = $dzbharr[$i]:''; 
	     		isset($dzchangjiaarr[$i])?$qv['dzchangjia'] = $dzchangjiaarr[$i]:''; 
	     		$qv['dzzushu'] = 1; 
	     		$data[] = $qv;	 
	     	} 
	  }
...
function arr2tree($tree, $rootId = 0,$single=null,$sid=0,$prcodeid=null,$scodeid=null) {
  $return = array(); 
  foreach($tree as &$leaf) { 
        $leaf['prcodeid']=$prcodeid;
    	if($scodeid)$leaf['scodeid']=$scodeid;
    if($leaf['pid'] == $rootId) { 
        ...
        if($subleaf['pid'] == $leaf['id']) { 
	        	if($leaf['note']=='single'){
		       		foreach ($single as $sk => $sv) {
	        			...
	        			$scodeid=$sv['codeid'];
	        			$leaf['children'][$sk]['children']=arr2tree($tree,$leaf['id'],$single,$sv['sid'],$prcodeid,$scodeid);
	        		}
	        	...
	        	if($leaf['note']=='songjian'){
	        		...
	        		$children=arr2tree($tree,$leaf['id'],$single,0,$prcodeid,$scodeid);
          ...
          $leaf['children'] = arr2tree($tree, $leaf['id'],$single,0,$prcodeid,$scodeid); 
          break; 
        } 
      } 
      $return[] = $leaf; 
...
//野狗
function wilddog_msg($tel,$content,$data=null,$date=null)
{
...
$sign_key = 'pGaBZugjPKul5tELCVTHmsHZ824zWyDiMNYpZmKO';//?
$sign_data = array('mobiles' => $tel, 'templateId' =>$content['templateId'], 'timestamp' => $time,
...
$post_data = array ('signature' => $signature,'mobiles' => $tel,"timestamp" => $time,"templateId" => $content['templateId'],"params"=>$content['params']);
$form_string= http_build_query($post_data);
p(http_curl($url,$form_string));
// // DEBUG
// echo "打印sign_str\n";
// var_dump($sign_str);
// echo "打印signature\n";
...
function test($tel,$code){
	...
	$sign_key = 'pGaBZugjPKul5tELCVTHmsHZ824zWyDiMNYpZmKO';
	$param='["'.$code.'"]';
	$sign_data = array('mobiles' => $mobile, 'templateId' => $templateId, 'timestamp' => $time,'params'=>$param);
	// 以字母升序(A-Z)排列
	ksort($sign_data);
	// var_dump($sign_data);
	$sign_str = http_build_query($sign_data) . '&' . $sign_key;
	...
	$url = "https://sms.wilddog.com/api/v1/${appId}/notify/send";
	// 不同接口参数不同， 详细参数请参见 https://docs.wilddog.com
	$post_data = array('signature' => $signature, "mobiles" => $mobile, "timestamp" => $time, "templateId" => $templateId,'params'=>$param);
	$form_string = http_build_query($post_data);
	// DEBUG
	// echo "打印sign_str\n";
	// var_dump($sign_str);
